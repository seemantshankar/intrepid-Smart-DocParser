...
func (fs *FlexibleString) UnmarshalJSON(data []byte) error {
	...
	if err := json.Unmarshal(data, &integer); err == nil {
		*fs = FlexibleString(strconv.FormatInt(integer, 10))
		return nil
	}
...
type ContractAnalysis struct {
	...
	SellerAddress       string                    `json:"seller_address"`
	SellerCountry       string                    `json:"seller_country"`
	TotalValue          FlexibleString            `json:"total_value"`          // Can handle both strings and numbers
	ContractStatedTotal FlexibleString            `json:"contract_stated_total"` // Can handle both strings and numbers
	Currency            string                    `json:"currency"`
	Milestones          []AnalysisMilestone       `json:"milestones" gorm:"-"`
	PaymentObligations  []PaymentObligation       `json:"payment_obligations" gorm:"-"`
	TaxAnalysis         *TaxAnalysis              `json:"tax_analysis" gorm:"-"`
	Calculations        *FinancialCalculations    `json:"calculations" gorm:"-"`
	Discrepancies       []FinancialDiscrepancy    `json:"discrepancies" gorm:"-"`
...
type AnalysisMilestone struct {
	Description       string         `json:"description"`
	Amount            FlexibleString `json:"amount"`            // Can handle both strings and numbers
	Percentage        float64        `json:"percentage"`
	CalculatedAmount  FlexibleString `json:"calculated_amount"` // Can handle both strings and numbers
...
// PaymentObligation represents a payment obligation extracted from the contract.
...
// TaxAnalysis represents tax calculations and analysis.
type TaxAnalysis struct {
	ApplicableTaxRate FlexibleString `json:"applicable_tax_rate"` // Can handle both strings and numbers
	TaxAmount         FlexibleString `json:"tax_amount"`
	NetAmount         FlexibleString `json:"net_amount"`
	TaxBasis          string         `json:"tax_basis"`
}

// FinancialCalculations represents detailed calculation breakdowns.
type FinancialCalculations struct {
	BaseAmounts          []FlexibleString `json:"base_amounts"`
	PercentagesApplied   []FlexibleString `json:"percentages_applied"`
	TaxCalculations      []FlexibleString `json:"tax_calculations"`
	FinalTotals          []FlexibleString `json:"final_totals"`
}

// FinancialDiscrepancy represents differences between contract and calculated amounts.
type FinancialDiscrepancy struct {
	Type              string         `json:"type"`
	ContractAmount    FlexibleString `json:"contract_amount"`
	CalculatedAmount  FlexibleString `json:"calculated_amount"`
	Difference        FlexibleString `json:"difference"`
